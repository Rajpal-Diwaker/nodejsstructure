...
const checkUser = async (criteria) => {
    ...
    try {
         const user = await knex('users').where('email',email).where('isActive','1').debug()
        return user;
    } catch (e) {
        return Promise.reject(e.toString());
    }
...
const checkCategoryName = async (criteria) => {
    const { name } = criteria
    try {
         const checkData = await knex('category').where('cateName',name).where('isActive','1').debug()
        return checkData;
    } catch (e) {
        return Promise.reject(e.toString());
    }
...
const addCategory = async (criteria) => {
    try {
        await knex("category").insert({cateName: criteria.name}).debug();
    } catch (e) {
        return Promise.reject(e.toString());
    }
}
...
const getCategory = async () => {
    try {
        const data = await knex('category').where('isActive','1').debug()
        return data;
    } catch (e) {
        return Promise.reject(e.toString());
    }
